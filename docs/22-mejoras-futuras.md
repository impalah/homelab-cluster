...
## 53. Komodo — visibilidad del clúster y API para operar los nodos, como stack Swarm sobre `postgres-main`

**Prioridad: media**

### Qué se busca

Instalar **Komodo** (`komo.do`, `moghtech/komodo`) con dos usos explícitos del usuario: (1) **ver el estado del clúster** (servidores, contenedores, stacks/servicios Swarm) en una UI, y (2) **llamar a su API para realizar operaciones** (desplegar/parar/reiniciar stacks y servicios, listar estado) desde scripts, n8n, Capataz o Forgejo Actions, en vez de SSH o de `docker stack deploy` a mano.

**Decisión del usuario (2026-09-20): sin MongoDB — reutilizar el `postgres-main` existente, añadiéndole las extensiones necesarias.** Este documento fija cómo hacerlo y, sobre todo, qué cuesta hacerlo así (la sección "Coste real" más abajo), porque `postgres-main` es el Postgres compartido por casi todo el clúster y aquí no se añade un servicio nuevo: se cambia la base de datos de todos los demás.

### Investigado (2026-09-20) — por qué encaja y cómo

- **Komodo v2 (marzo 2026) trae soporte Swarm nativo**: recursos de tipo *Swarm* (nodos, servicios, tareas, stacks, configs, secrets) agrupados sobre *Servers*; los stacks Swarm se despliegan con `docker stack deploy`, no con `docker compose`. **Solo hace falta el agente (Periphery) en los nodos manager** — Core enruta las peticiones Swarm a cualquier manager disponible y reintenta con el siguiente si uno falla. Este clúster tiene los 5 managers (`retaco`/`pi-obs`/`pi-sonar`/`pi-utils`/`pinchi`), así que hay redundancia natural.
- **Arquitectura**: *Core* (API + UI, puerto 9120) + *Periphery* (agente, uno por servidor) + base de datos. Periphery soporta `x86_64` y `aarch64` (los Pi, sin problema). La documentación de Komodo solo describe desplegar Periphery en los managers de un Swarm, **no** desplegar Core como servicio Swarm — es una combinación menos rodada que hay que validar (fase 0), no una configuración documentada oficialmente.
- **Conexión Periphery→Core saliente (v2)**: Periphery puede iniciar la conexión hacia Core (`PERIPHERY_CORE_ADDRESS`) con autenticación por par de claves (clave de *onboarding* de un solo uso, luego cada Periphery genera su propio par y solo comparte la pública). Consecuencia buena para este clúster: **ningún nodo necesita abrir el puerto 8120 hacia la LAN** — todo el tráfico entra por Core detrás de Traefik (`wss://` sobre el mismo hostname), a diferencia de `portainer-agent`, que sí publica su puerto.
- **API**: HTTP tipo RPC — `POST /read/<Petición>`, `/write/…`, `/execute/…` con cuerpo JSON. Autenticación por cabeceras `X-Api-Key` + `X-Api-Secret`; las claves se crean desde la UI para un usuario o para un **service user** con permisos acotados. Clientes oficiales en TypeScript y Rust (más uno de PyPI y un MCP no oficial de terceros — sin auditar, no se considera de momento). Confirmar en la fase 0 el formato exacto de cada petición contra la versión desplegada.
- **Autenticación de personas**: soporta OIDC (encaja con Authentik, mejora 29) y usuario/contraseña local; `KOMODO_DISABLE_USER_REGISTRATION=true` desde el principio.
- **Conflicto real con `apikey-service`**: el middleware `apikey-auth` de Traefik valida la cabecera `X-Api-Key`, la misma que usa Komodo para su propia API. **El router de Komodo NO debe llevar ese middleware** (mismo criterio que `registry`/`ntfy`: auth propia, cabeceras propias) — si se pusiera, `apikey-auth` intentaría validar la clave de Komodo contra su base de datos y rechazaría todas las llamadas.

### Base de datos: FerretDB v2 sobre `postgres-main` con la extensión DocumentDB

Komodo solo habla el protocolo de MongoDB. Para que use Postgres hace falta **FerretDB v2** (un contenedor pequeño, escrito en Go, que traduce el protocolo de Mongo) sobre un Postgres con la extensión **DocumentDB** (de Microsoft, empaquetada por FerretDB) — es la alternativa que la propia documentación de Komodo describe ("FerretDB"). Datos comprobados en vivo el 2026-09-20:

- **Imagen oficial** `ghcr.io/ferretdb/postgres-documentdb:<pg>-<documentdb>-ferretdb-<ferretdb>`, publicada para **PG 15, 16 y 17** (la última, `0.107.0-ferretdb-2.7.0`, de noviembre de 2025) — **no hace falta subir de PG 16**, así que se evita un salto de versión mayor. Inspeccionada: Debian 12 (bookworm), PostgreSQL 16.10, **2,3 GB** (trae también PostGIS, pgvector, pg_cron y RUM), mismo entrypoint oficial de `postgres` (`POSTGRES_PASSWORD_FILE`, `PGDATA`…). En el primer `initdb` añade a `postgresql.conf` `shared_preload_libraries = 'pg_cron,pg_documentdb_core,pg_documentdb'`, `cron.database_name = 'postgres'` y ocho parámetros `documentdb.*`, y ejecuta `CREATE EXTENSION documentdb CASCADE` en la BD `postgres`.
- **`postgres-main` hoy** (`docker-swarm/stacks/postgres-main/`): PostgreSQL 16.15 sobre **Alpine/musl**, en `retaco`, 9 bases de datos (~430 MB en total), 46 de 100 conexiones en uso, `shared_preload_libraries` vacío y ninguna de las extensiones necesarias disponible. Todas las bases declaran colación `en_US.utf8`.
- **Los paquetes `.deb` de DocumentDB para instalar sobre un Postgres propio están compilados para Debian/glibc; no hay build para Alpine/musl.** Es decir: **no se puede "añadir la extensión" a la imagen `postgres:16-alpine` actual — hay que cambiar la base de la imagen a Debian.** Ese es el hecho que convierte "añadir unas extensiones" en una migración de `postgres-main`.
- FerretDB necesita la extensión creada en la base que indica `cron.database_name` (pg_cron solo trabaja en **una** BD por clúster). La documentación y la imagen usan `postgres`; una BD propia (`komodo`) sería más limpia y coherente con "una BD por servicio" (`create-postgres-db.sh`) — comprobar en la fase 0 que funciona.

### Coste real de usar `postgres-main` — lo que cambia para TODOS sus inquilinos

1. **musl → glibc: la colación cambia, y con ella el orden de los índices de texto.** Bajo musl, `en_US.utf8` en la práctica ordena por punto de código (musl no implementa colación por idioma); bajo glibc, `en_US.utf8` sí ordena por idioma. **Reutilizar el directorio de datos actual con la imagen Debian corrompería en silencio los índices de texto** de las 9 bases. Por eso el plan NO reutiliza el directorio de datos: volcado lógico (`pg_dumpall`) y restauración en un clúster nuevo creado con `--locale=C.UTF-8` (orden por punto de código, el más fiel al comportamiento actual). Verificar en el ensayo comparando resultados de `ORDER BY` sobre columnas de texto reales.
2. **Ventana de mantenimiento con todos los dependientes parados** (deducidos de las bases y roles reales): `apikeys` → `apikey-service` (**ojo: es el `forwardAuth` de Traefik para los servicios protegidos con API key — durante la ventana esos endpoints fallan**), `authentik` (SSO de Capataz y Prometheus), `bifrost`, `capataz`, `forgejo` (y con él los runners de CI), `n8n`, `openwebui`, `sonarqube`, `postgres_exporter`, y `content_pipeline` (rol `content_pipeline_user`: **consumidor por identificar antes de la ventana** — solo aparece mencionada en `docs/05` y `docs/31`, ningún stack de este repo la usa; probablemente un flujo de n8n, confirmarlo). Los datos son pequeños (~430 MB): volcado y restauración de minutos, no de horas.
3. **Librerías precargadas para todo el clúster**: `pg_cron` y los workers de DocumentDB arrancan con el servidor, para todas las bases; un fallo de la extensión (o un `restart` mal hecho) afecta a todos los inquilinos, no solo a Komodo. Se mitiga con el ensayo previo (fase 0) y con el plan de vuelta atrás (punto 7).
4. **Parcheo**: la imagen oficial va por detrás de la que corre hoy (PostgreSQL 16.10 frente a 16.15) y se publica al ritmo de FerretDB. Alternativa: imagen derivada de `postgres:16-bookworm` + paquetes (parches de PG al día, pero más mantenimiento propio y sin `.deb` oficial para bookworm — los publicados son de Debian 11). **Por defecto, la imagen oficial** (la combinación que el propio proyecto prueba); se reevalúa en la fase 0.
5. **Presupuesto de conexiones**: 46/100 en uso hoy (SonarQube 20, Authentik 8…); FerretDB abre su propio pool. Medir en el ensayo y subir `max_connections` si hace falta.
6. **Lo que no cambia**: `backup-postgres.sh`/`restore-postgres.sh` (`pg_dump` existe en la imagen Debian), `postgres-exporter` de `pi-obs`, el alias `postgresql.404labo.net:5432`, el rol `dbadmin` y los secretos actuales.
7. **Vuelta atrás**: el directorio de datos antiguo no se toca (el nuevo va en otra ruta) y el compose actual con `postgres:16-alpine` se conserva; volver = volver a desplegar el compose anterior. Se conserva durante unos días tras la migración. Nunca `chown -R` sobre el directorio de datos (incidente real ya documentado en `docs/13-troubleshooting.md`).
8. **Plan B, si el ensayo falla o el riesgo no compensa**: un Postgres dedicado `postgres-komodo` con la misma imagen (precedente exacto: `postgres-infisical`, ADR 0002) — mismo Core y mismo FerretDB, cero impacto sobre los inquilinos actuales. Es barato de tener a mano porque comparte todo el trabajo de las fases 2 en adelante.

### Decisión de diseño

1. **Core + FerretDB como stack Swarm `komodo`**, nace ya como Swarm (sin legado), **sin MongoDB**: todo el estado vive en Postgres, así que ni Core ni FerretDB necesitan `constraints` de nodo; solo el volumen de claves de Core y sus copias de seguridad de la BD (Core las genera solo, con fechas) van en NFS (`driver: local` + `type: nfs`, `vers=3`, patrón de la mejora 47). FerretDB llega a Postgres por el alias de la LAN (`postgresql.404labo.net:5432`, convención del clúster para acceso cross-host). Router de Traefik por `deploy.labels` en `komodo.404labo.net`, con soporte WebSocket (Traefik ya lo hace de serie). Registro DNS + fila en el `README.md` cuando se implemente.
2. **`postgres-main` pasa a la imagen con DocumentDB** (imagen oficial, PG 16, por defecto) mediante `command: postgres -c shared_preload_libraries=… -c cron.database_name=… -c documentdb.*=…` en el propio compose (en vez de editar el `postgresql.conf` dentro del directorio de datos), con directorio de datos **nuevo, local y no NFS** (mejora 47 mantiene fuera a las bases de datos activas), creado con `--locale=C.UTF-8`. Rol propio `komodo` (no superusuario si FerretDB lo admite — comprobar en el ensayo) y BD propia.
3. **Periphery como contenedor, no como servicio `systemd`** (la instalación por script Python es la que Komodo recomienda por defecto): en modo contenedor es independiente del sistema init — sobrevive sin cambios a la migración de los Pi a Alpine/OpenRC (mejora 51), donde el script `systemd` no serviría. En los 5 managers: **servicio `global` con `node.role == manager`** en un stack aparte (`komodo-periphery`), con `docker.sock`, `/proc` y `/etc/komodo` montados (esta última con la **misma ruta dentro y fuera** del contenedor, requisito de Komodo). `docker.sock` da control total del host — mismo riesgo ya aceptado para `portainer-agent`.
4. **`ryzen` y `pi-dns` (fuera del Swarm) como servidores normales** con Periphery en Compose clásico, una vez validado lo anterior. `ryzen` se apaga habitualmente — aparecerá *offline* en Komodo cuando esté dormido; silenciar/ignorar sus alertas de caída en vez de tratarlo como incidencia.
5. **Autogestión: los stacks `komodo` y `komodo-periphery` se despliegan siempre a mano** (`docker stack deploy`), nunca desde el propio Komodo — mismo motivo que Portainer no se gestiona a sí mismo: si Core se cae, la herramienta para levantarlo no puede depender de él. **Lo mismo vale para `postgres-main`**: Komodo depende de él, así que Komodo nunca debe ser el mecanismo para reiniciarlo o redesplegarlo.
6. **Convive con Portainer y Capataz, no los sustituye de entrada.** Komodo cubre Swarm + API de operaciones; Portainer (`pi-utils`) sigue como está y Capataz sigue siendo la consola de estado/automatización propia. Valorar más adelante si Komodo hace redundante a Portainer — decisión posterior, con datos de uso real.
7. **Seguridad de la API**: la API de Komodo con permisos de ejecución equivale a control remoto de todos los nodos (Periphery ejecuta como root con `docker.sock`). Mitigaciones acordadas de antemano: **service users con permisos mínimos por caso de uso** (uno de solo lectura para paneles/estado, otros por acción concreta), nunca la clave del admin en scripts; terminales de Periphery deshabilitados (`PERIPHERY_DISABLE_TERMINALS=true`) salvo necesidad real; Komodo solo accesible desde la LAN/Tailscale (como el resto de `*.404labo.net`); claves de la API guardadas en Infisical (proyecto propio, carpeta `/komodo/`), no en ficheros del repo.

### Secretos

Contraseña del rol `komodo` de Postgres, `KOMODO_JWT_SECRET`, `KOMODO_WEBHOOK_SECRET` y credenciales del admin inicial — nunca en claro en el compose (patrón del clúster: `docker secret` + Infisical). A comprobar en la fase 0 cuál de los dos mecanismos admiten las imágenes de Core y de FerretDB: variables `*_FILE` con `docker secret` montado (lo más simple si están soportadas), o `infisical login` + `infisical run` delante del entrypoint (solo si la imagen trae shell — ver la lista de imágenes "bloqueadas por falta de shell" en la mejora 28). Punto abierto concreto: FerretDB recibe la contraseña de Postgres dentro de una URL (`FERRETDB_POSTGRESQL_URL`), lo que complica el patrón `_FILE` — si su imagen no trae shell, habrá que resolverlo (plan: URL sin contraseña + mecanismo alternativo de FerretDB, o un entrypoint mínimo propio). Nota heredada de un despliegue Swarm de terceros: un `env_file` **no** se resuelve bien con `docker stack deploy` (referencias `${VAR}` sin expandir y valores de intervalo mal serializados) — las variables van directamente en la sección `environment` del compose.

### Plan de ejecución por fases

**Fase 0 — Ensayo aislado, sin tocar `postgres-main` ni el clúster.** En `ryzen` (o `pinchi`): (1) levantar la imagen oficial PG 16 con `POSTGRES_INITDB_ARGS=--locale=C.UTF-8`; (2) restaurar en ella un `pg_dumpall` reciente de `postgres-main` (`backup-postgres.sh`) y comprobar: restauración sin errores, mismos recuentos de filas por tabla, mismo resultado de `ORDER BY` sobre columnas de texto reales (Authentik, n8n, SonarQube), `ANALYZE`; (3) añadir las librerías precargadas y crear la extensión en una BD propia `komodo` con `cron.database_name` apuntando a ella (si no funciona, usar `postgres` como hace la imagen); (4) crear el rol `komodo` sin superusuario y comprobar que FerretDB funciona con él; (5) levantar FerretDB (versión pareada con la de DocumentDB, telemetría desactivada) y un Core de prueba con tag fijado de v2 — que arranque, cree el admin y admita un Periphery de prueba; (6) medir conexiones y memoria; (7) decidir imagen oficial vs derivada y PG 16 vs 17 (por defecto 16, oficial). Además, las verificaciones ya previstas de Komodo/Swarm: mecanismo de secretos de cada imagen; conexión saliente de Periphery `wss://` a través de Traefik (WebSocket); y **cómo se identifica cada réplica de un servicio `global`** (un mismo `--connect-as`/clave de *onboarding* para las 5 tareas no sirve tal cual: hace falta que cada nodo registre su propio nombre y persista su propia clave en un directorio local del nodo — no está documentado para Swarm). Si un Periphery `global` resulta inmanejable, plan B: un servicio Swarm por manager con `constraints: node.hostname == <nodo>` (5 definiciones cortas). Fijar versiones exactas de todas las imágenes (misma norma que Forgejo).

**Fase 1 — Migrar `postgres-main` a la imagen con DocumentDB (la parte de riesgo real).** Requisitos previos: ensayo de la fase 0 limpio; backup verificado **y** copia fuera de `retaco` (mejora 1); consumidor de `content_pipeline` identificado; aviso de la ventana de mantenimiento. Pasos: (1) parar los dependientes de la lista de arriba (`docker service scale <servicio>=0`), dejando `postgres-main` como último; (2) volcado final (`pg_dumpall --globals-only` + `pg_dump` en formato custom por base); (3) nuevo stack file de `postgres-main` con imagen nueva y directorio de datos **nuevo** (p. ej. `/srv/homelab/retaco/postgres/data-documentdb`, local), `initdb` con `--locale=C.UTF-8`, mismo `constraints: node.hostname==retaco`, mismo puerto y mismo secreto de `dbadmin`; (4) restaurar globals (roles y contraseñas) y cada base; (5) crear la extensión y el rol/BD de Komodo; (6) levantar los dependientes **de uno en uno** verificando su healthcheck (`apikey-service` y `authentik` los primeros — de ellos cuelgan otros servicios), y comprobar métricas en `postgres-exporter`; (7) dejar el directorio de datos antiguo y el compose anterior intactos durante unos días — ese es el rollback. Al terminar, actualizar `docs/05-instalacion-retaco.md`, la mejora 27 (TLS en `postgres-main`: la imagen Debian soporta `ssl = on` igual) y toda referencia a `postgres:16-alpine`.

**Fase 2 — FerretDB + Core (`docker-swarm/stacks/komodo/`).** Stack Swarm con el diseño de arriba; DNS `komodo.404labo.net` en Pi-hole; primer login con el admin local; registro de usuarios deshabilitado. Verificar que la UI carga, que Core persiste en Postgres (comprobar que aparecen los datos en el esquema de DocumentDB) y que un Periphery de prueba en un solo nodo aparece como servidor.

**Fase 3 — Periphery en los 5 managers (`docker-swarm/stacks/komodo-periphery/`).** Los 5 aparecen como *Servers*; crear el recurso *Swarm* de Komodo que los agrupa como managers. Comprobar que la UI muestra nodos, servicios y tareas reales del clúster y que la caída de un manager (probar apagando uno con calma) no rompe la vista — Core reintenta con otro.

**Fase 4 — `ryzen` y `pi-dns`.** Periphery en Compose clásico (`ryzen/docker-compose.komodo-periphery.yml`, `pi-dns/docker-compose.yml`), como servidores fuera del Swarm. En `pi-dns`, con cuidado: es el único DNS del clúster (mejora 40 pendiente) — solo añadir un contenedor de solo lectura funcional, sin tocar Pi-hole/Unbound.

**Fase 5 — API.** Crear en la UI los service users (uno de solo lectura, otro con permisos de ejecución acotados), guardar sus claves en Infisical (`/komodo/`), y probar desde fuera de la UI: listar servidores/servicios (`POST /read/…`) y una operación de ejecución inocua (reiniciar un servicio sin estado). Documentar cada llamada validada. Opcional a partir de aquí: script envoltorio en `shared/scripts/` que tome la clave de Infisical (mismo patrón que el envoltorio SSH de la mejora 52).

**Fase 6 — Integraciones.** Alerter de Komodo hacia **ntfy** (mejora 4, ya operativa) para caídas de servidores/contenedores, coordinado con la mejora 42 (alertas de disponibilidad) para no duplicar avisos. **OIDC con Authentik** (mejora 29) para las personas, dejando el admin local solo como cuenta de emergencia. Conector de Komodo en Capataz (catálogo v2, "conectores y recursos", `docs/28`) si compensa unificar la vista.

**Fase 7 (opcional, posterior) — GitOps.** Komodo puede desplegar stacks desde un repositorio Git con *webhooks* y declarar sus propios recursos como TOML versionado — encaja con **Forgejo** (mejora 7, ya desplegado) y con la mejora 34 (GitOps, propuestas a evaluar): un push a `docker-swarm/stacks/<x>/` podría disparar el redespliegue. Requiere resolver antes cómo pasar `--with-registry-auth` y los `docker config`/`secret` externos (`-vN`) que hoy exige `CLAUDE.md` para desplegar cada stack — no asumir que funciona sin probarlo. Fuera del alcance de esta mejora si no se pide explícitamente.

### Documentación al implementar

Documento numerado nuevo `docs/39-komodo.md` (arquitectura, comandos exactos, incidentes reales) + fila en el índice de `docs/01-topologia.md`, fila en el `README.md` (hostname `komodo.404labo.net`), `shared/dns/dns-records.md`, y actualizar `docs/05-instalacion-retaco.md` (nueva imagen de `postgres-main`). Copias de seguridad: las bases de Komodo viven en `postgres-main`, así que entran en `backup-postgres.sh`; Core además genera sus propias copias con fechas, que conviene incluir en la mejora 1 (copias fuera del nodo).

### Esfuerzo estimado

**Medio-alto** (era medio con MongoDB) — el grueso ya no es Komodo sino la **fase 1**, una migración de la base de datos compartida con ventana de mantenimiento y un cambio de imagen base (Alpine→Debian) que toca a 9 bases y a casi todos los servicios del clúster. A cambio se evita añadir un motor de base de datos nuevo. Incertidumbre técnica concreta, resuelta en la fase 0: que FerretDB funcione con un rol sin superusuario y con una BD propia, cómo pasarle la contraseña sin dejarla en claro, Core como servicio Swarm y un Periphery `global` con identidad propia por nodo (ninguna de las dos últimas descrita en la documentación oficial). Dependencias: se apoya en Traefik/DNS/Infisical ya operativos; **conviene tener antes la mejora 1** (copias de seguridad fuera del nodo) y se beneficia de ntfy (mejora 4), Authentik (mejora 29) y Forgejo (mejora 7). Si la fase 0 o la 1 se tuercen, el plan B (`postgres-komodo` dedicado) desbloquea el resto sin tocar a nadie.
...
| 53 | Komodo — visibilidad del clúster + API para operar los nodos, como stack Swarm con FerretDB sobre `postgres-main` | Media | Medio-alto | Komodo v2 con soporte Swarm nativo (investigado 2026-09-20). **Decisión del usuario: sin MongoDB, en `postgres-main`** — exige cambiar su imagen de Alpine/musl a Debian con la extensión DocumentDB (migración de la BD compartida con ventana de mantenimiento, fase 1); plan B: Postgres dedicado. Conviene antes la mejora 1 (copias fuera del nodo); se beneficia de ntfy (4), Authentik (29), Forgejo (7) y GitOps (34) |
