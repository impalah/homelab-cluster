...
from markitdown_service.logging_setup import InterceptHandler, setup_logging


def test_setup_logging_configures_a_single_stdout_sink() -> None:
    setup_logging(Settings(markitdown_log_level="INFO"))
    setup_logging(Settings(markitdown_log_level="DEBUG"))  # no debe acumular sinks

    logger.info("mensaje de prueba tras reconfigurar logging")


def test_setup_logging_intercepts_stdlib_logging() -> None:
    setup_logging(Settings(markitdown_log_level="DEBUG"))
