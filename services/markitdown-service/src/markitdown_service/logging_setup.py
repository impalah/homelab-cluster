...
def setup_logging(settings: Settings) -> None:
    ...
    logger.add(
        sys.stdout,
        level=settings.markitdown_log_level,
        serialize=settings.markitdown_log_format == "json",
        enqueue=True,
    )
    ...
    logging.getLogger("uvicorn.access").setLevel(
        logging.DEBUG if settings.markitdown_log_level.upper() == "DEBUG" else logging.WARNING
    )
